...
**v1.6 — Live Hermes + Obsidian Shared Memory** — native memory deployed across local project repos; Stage 2 cost observability remains next
...
2026-08-14 — Hermes + Obsidian `v1.6.0-alpha.1` portable rollout — **validated locally** (native memory, 28 Hermes projects, path-portable launcher/installer, Codex MCP bridge, Obsidian vault)
...
- Historical Gate A/B requirements were satisfied by the pinned reversible install and subsequent live shared-memory rollout; future production expansion requires a new approval.
...
- **Product pin checkpoint 2026-08-09 (ADR-0014):** NousResearch/hermes-agent tag `v2026.8.3` → commit `3c27eb62…` (SSH-signed tag, MIT); at that preflight checkpoint nothing was installed. Superseded operationally by the approved live local deployment on 2026-08-14.
...
- **DOCUMENT CENTER FULL READ-ONLY AUDIT 2026-08-09 — verdict FAIL (CRIT 2/HIGH 2/MED 4/LOW 2):** F-02 124/124 authenticated SharePoint URLs leaked in public registry (P0), F-03 invalid committed checksum, F-04 reconciliation metadata missing, F-09 known-failing artifact waved off; baseline 627/124/503 confirmed; staging pipeline sound; governance decision **REUSE_EXISTING_SITE_WITH_CONDITIONS**; **no Document Center changes made**; evidence `DOCUMENT_CENTER_FULL_READONLY_AUDIT_2026-08-09.md`
- **RAE-Document-Center (OneDrive) path added 2026-08-09:** `G:\GreenData_Res\OneDrive - Maejo university\RAE-Document-Center` — 8 FY2569 xlsx in `07-GreenOffice\resource\` (updated 2026-08-07) may resolve GOFFICE2026 FY2569 pending-data finding (owner-side re-import decision); recorded in `LOCAL_INTEGRATION_VALIDATION_REPORT.md` §9; read-only, untouched
...
- Prior tag `v1.5.0-alpha.1` → `b995f19`; `v1.6.0-alpha.1` release PR/tag are the current publication work
