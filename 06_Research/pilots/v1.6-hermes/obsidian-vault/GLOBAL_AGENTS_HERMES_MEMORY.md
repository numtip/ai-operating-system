...
- Before a task that matches a reusable workflow, call `skills_list` and load the relevant native Hermes `SKILL.md` with `skill_view`. For Obsidian work, load the `obsidian` skill; for Hermes configuration or orchestration, load the `hermes-agent` skill.
