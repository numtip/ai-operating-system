...
- **Budget enforcement** — two-tier, via optional `context_budget` in a model profile:
  - `preferred_max_files` (soft target for optional refs; default `min(profile.context_limit_policy.max_context_refs, 10)`; legacy `max_files` is treated as the preferred limit when `preferred_max_files` is absent).
  - `hard_max_files` (absolute cap on total selected refs; default = `preferred_max_files`). Required refs are never dropped, so a required count above the hard cap is reported as a `hard_cap_breached` warning rather than dropped; operators must adjust policy in that case.
  - `max_tokens` (default 0 = off; when > 0, optional refs cost path+source+reason+16 chars toward `max_tokens * 4`).
  - Over-budget refs are reported in `metrics.optimization.rejected`; budget values are reported as `metrics.optimization.budget_preferred_max_files` / `budget_hard_max_files`.
  - **Bounded pilot exception (GOFFICE2026, 2026-08-09):** `deepseek-v4-flash` uses `preferred_max_files=6` / `hard_max_files=8` so the pilot's 8 mandatory refs are admissible. This is bounded: (a) hard cap still bounds the total manifest, (b) required refs remain the only path past the preferred limit (optional refs still capped at 6), (c) the cap was raised only for this pilot profile, not globally, and (d) any required-count > 8 would surface as `hard_cap_breached` instead of being silently dropped.
