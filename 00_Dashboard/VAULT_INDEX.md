# AI-OS Vault Index

> **Purpose**: This note is the Obsidian entry point for the AI-OS local vault.
> The vault root is the repository itself — repo Markdown is the **source of truth**.
> This index only **links** to canonical docs; it does not duplicate their content.

## Architecture

- [AI_OPERATING_SYSTEM_BLUEPRINT_V4.1_VALIDATED_OPERATING_BASELINE.md](../AI_OPERATING_SYSTEM_BLUEPRINT_V4.1_VALIDATED_OPERATING_BASELINE.md) — validated operating baseline (supersedes v4.0 operational sections)
- [AI_OPERATING_SYSTEM_BLUEPRINT_V4.md](../03_Architecture/AI_OPERATING_SYSTEM_BLUEPRINT_V4.md)
- [ARCHITECTURE_OVERVIEW.md](../03_Architecture/ARCHITECTURE_OVERVIEW.md)
- [ROADMAP.md](../03_Architecture/ROADMAP.md)

## Memory

- [CURRENT_STATE.md](../07_Memory/CURRENT_STATE.md)
- [SYSTEM_MEMORY.md](../07_Memory/SYSTEM_MEMORY.md)

## Research — Hermes Pilot (v1.6)

- [DOCUMENT_CENTER_FULL_READONLY_AUDIT_2026-08-09.md](../06_Research/pilots/v1.6-hermes/DOCUMENT_CENTER_FULL_READONLY_AUDIT_2026-08-09.md) — Document Center full read-only audit (2026-08-09)
- [GOFFICE2026_FULL_READONLY_AUDIT_2026-08-09.md](../06_Research/pilots/v1.6-hermes/GOFFICE2026_FULL_READONLY_AUDIT_2026-08-09.md) — full read-only audit (2026-08-09)
- [LOCAL_RUNTIME_REPORT.md](../06_Research/pilots/v1.6-hermes/LOCAL_RUNTIME_REPORT.md) — real DeepSeek pilot result (2026-08-09)
- [LOCAL_PILOT_REPORT.md](../06_Research/pilots/v1.6-hermes/LOCAL_PILOT_REPORT.md) — install/smoke/rollback record
- [LOCAL_INTEGRATION_VALIDATION_REPORT.md](../06_Research/pilots/v1.6-hermes/LOCAL_INTEGRATION_VALIDATION_REPORT.md)
- [HERMES_PREFLIGHT_REPORT.md](../06_Research/pilots/v1.6-hermes/HERMES_PREFLIGHT_REPORT.md)
- [goffice2026/STAGE-1-RESULTS.md](../06_Research/pilots/v1.6-hermes/goffice2026/STAGE-1-RESULTS.md)

## Housekeeping

- `.obsidian\` and `.runtime\` must be gitignored (workspace/cache, not source of truth).
- Repo docs remain the canonical source; any vault-side customization belongs in `.obsidian\` only.
